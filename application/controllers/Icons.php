...
class Icons extends CI_Controller {
	...
	public function create(){
		...
		$master['iconname'] 	= $this->input->post('iconname');
		$master['icon'] 		= $this->input->post('icon');
		...
		$this->form_validation->set_rules('iconname', 'Icon Name', 'trim|xss_clean|required');
		...
		if($this->form_validation->run()) {
			$result = $this->icons_model->create($master,$id);
	...
	public function getlist($param1 = '') {
        $results = $this->icons_model->getlist();
        $data 	 = array();
		$no 	 = 0;
        foreach ($results  as $r) {
			...
			$row[]  = '<a class="btn btn-info GetDetail" data-id="'.$id.'" href="javascript:void(0)"> Edit</a> <a class="btn btn-danger delete" data-id="'.$id.'" data-module="icon" data-title="Icons" href="javascript:void(0)" data-modal="confirmation"> Delete</a>';	
			$data[] = $row;
        }
    ...
    public function getById() {
    	$id 	 = $this->input->post('id');
        $results = $this->icons_model->getById($id);
        $data 	 = array();
