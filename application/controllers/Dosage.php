...
class Dosage extends CI_Controller {
	...
	public function create(){
		...
		$master['dosage'] 		= $this->input->post('dosage');
		...
		if($this->form_validation->run()) {
			$result = $this->dosage_model->create($master,$id);
	...
	public function getlist($param1 = '') {
        $results = $this->dosage_model->getlist();
        $data 	 = array();
		$no 	 = 0;
        foreach ($results  as $r) {
			...
			$row[]  = '<a class="btn btn-info GetDetail" data-id="'.$id.'" href="javascript:void(0)"> Edit</a> <a class="btn btn-danger delete" data-id="'.$id.'" data-module="dosage" data-title="Dosage" href="javascript:void(0)" data-modal="confirmation"> Delete</a>';
			$data[] = $row;
        }
    ...
    public function getById() {
    	$id 	 = $this->input->post('id');
        $results = $this->dosage_model->getById($id);
        $data 	 = array();
