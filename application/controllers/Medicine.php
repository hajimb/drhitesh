...
class Medicine extends CI_Controller {
	...
	public function create(){
		...
		$master['medicine'] = $this->input->post('medicine');
		...
		$this->form_validation->set_rules('medicine', 'Medicine Name', 'trim|xss_clean|required');
		...
		if($this->form_validation->run()) {
			$result = $this->medicine_model->create($master,$id);
	...
	public function getlist($param1 = '') {
        $results = $this->medicine_model->getlist();
        $data 	 = array();
		$no 	 = 0;
        foreach ($results  as $r) {
			...
			$row[]  = '<a class="btn btn-info GetDetail" data-id="'.$id.'" href="javascript:void(0)"> Edit</a> <a class="btn btn-danger delete" data-id="'.$id.'" data-module="medicine" data-title="Medicine" href="javascript:void(0)" data-modal="confirmation"> Delete</a>';	
			$data[] = $row;
        }
    ...
    public function getById() {
    	$id 	 = $this->input->post('id');
        $results = $this->medicine_model->getById($id);
        $data 	 = array();
