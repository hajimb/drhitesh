...
class Potency extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('potency_model');
		$this->load->helper('security');
		$this->heading = 'Potency';
	}


	public function index(){
	    $page_data['heading']    = 'Sexologist $this->heading Lists';
	    $page_data['pagename']   = 'potency';
	...
	public function create(){
		...
		$master['potency'] 		= $this->input->post('potency');
		...
		$this->form_validation->set_rules('potency', 'Potency', 'trim|xss_clean|required');
		...
		if($this->form_validation->run()) {
			$result = $this->potency_model->create($master,$id);
	...
	public function getlist($param1 = '') {
        $results = $this->potency_model->getlist();
        $data 	 = array();
		$no 	 = 0;
        foreach ($results  as $r) {
			...
			$row[]  = '<a class="btn btn-info GetDetail" data-id="'.$id.'" href="javascript:void(0)"> Edit</a> <a class="btn btn-danger delete" data-id="'.$id.'" data-module="potency" data-title="Potency" href="javascript:void(0)" data-modal="confirmation"> Delete</a>';		
			$data[] = $row;
        }
    ...
    public function getById() {
    	$id 	 = $this->input->post('id');
        $results = $this->potency_model->getById($id);
        $data 	 = array();
