...
class Sexologist extends CI_Controller {	
...
if($id ==0){	
	...
	public function answering(){
		...
		$this->form_validation->set_rules('answer_text', 'Answer', 'trim|xss_clean|required');		
	...
	public function getquestionlist($param1 = '') {
        ...
        foreach ($results  as $r) {
			...
			$row[]  = substr($r['question'], 0, 50);
			$row[]  = $r['ispublic'];
			$row[]  = '<a class="btn btn-success" href="'.base_url().'question/answer/'.$id.'"> Answer</a> <a class="btn btn-info view" data-id="'.$id.'" href="javascript:void(0)"> View</a>';
			$data[] = $row;
        }
    ...
    public function getQuestion(){
        ...
        $page =  $_GET['page'];
        $result = '';
        $questions = $this->question_model->getquestionlist('all',$page,'public',1);
        foreach($questions as $question){
        	//$php_timestamp_date = date("d F Y H:i:s", $question['added_date']);
            $result .='<h2><a href="#">'.$question['question'].'</a></h2>';
            $result .='<p class="lead"> by <a href="#">'.$question['name'].'</a></p>';
            $result .='<p><span class="glyphicon glyphicon-time"></span> Posted on August 28, 2013 at 10:00 PM</p>';
