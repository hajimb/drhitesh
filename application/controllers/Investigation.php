...
class Investigation extends CI_Controller {
	...
	public function __construct(){
		...
		$this->heading = 'Investigation';
	}


	public function index(){
	    $page_data['heading']    = 'Sexologist $this->heading Lists';
	    $page_data['pagename']   = 'investigation';
	...
	public function create(){
		...
		$master['investigation'] 		= $this->input->post('investigation');
		...
		$this->form_validation->set_rules('investigation', 'Investigation', 'trim|xss_clean|required');
		...
		if($this->form_validation->run()) {
			$result = $this->investigation_model->create($master,$id);
	...
	public function getlist($param1 = '') {
        $results = $this->investigation_model->getlist();
        $data 	 = array();
		$no 	 = 0;
        foreach ($results  as $r) {
			...
			$row[]  = '<a class="btn btn-info GetDetail" data-id="'.$id.'" href="javascript:void(0)"> Edit</a> <a class="btn btn-danger delete" data-id="'.$id.'" data-module="investigation" data-title="Investigation" href="javascript:void(0)" data-modal="confirmation"> Delete</a>';	
			$data[] = $row;
        }
    ...
    public function getById() {
    	$id 	 = $this->input->post('id');
        $results = $this->investigation_model->getById($id);
        $data 	 = array();
