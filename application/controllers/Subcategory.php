...
class Subcategory extends CI_Controller {
	...
	public function __construct(){
		...
		$this->heading = 'Subcategory';
	}

	public function index(){
	    $page_data['heading']    = 'Sexologist $this->heading Lists';
	    $page_data['pagename']   = 'subcategory';
	    $page_data['category']   = getcategory();
	...
	public function create(){
		...
		$master['cid'] 		    = $this->input->post('cid');
		$master['subcategory'] 	= $this->input->post('subcategory');
		...
		$this->form_validation->set_rules('cid', 'Category', 'trim|xss_clean|required');
		$this->form_validation->set_rules('subcategory', 'Subcategory', 'trim|xss_clean|required');
		...
		if($this->form_validation->run()) {
			$result = $this->subcategory_model->create($master,$id);
	...
	public function getlist($param1 = '') {
        $results = $this->subcategory_model->getlist();
        $data 	 = array();
		$no 	 = 0;
    ...
    public function getById() {
    	$id 	 = $this->input->post('id');
        $results = $this->subcategory_model->getById($id);
        $data 	 = array();
