...
class Physicalexamination_model extends CI_Model {
	private $table;
	public function __construct(){
		parent::__construct();
		$this->table  = 'master_physical_examination';
	}
		...
		if($isexists==0){
			if($id ==0){
				...
				if($this->db->trans_status() === FALSE){
				    ...
				    $result = array("status" => true,"success" => true,"msg" => "Physical Examination  Successfully Added");
				}	
			}else{
				$this->db->where('id', $id);
				...
				if($this->db->trans_status() === FALSE){
				    ...
				    $this->db->trans_commit();
				    $result = array("status" => true,"success" => true,"msg" => "Physical Examination  Successfully Updated");
				}	
			}
		}else{
			$result = array("status" => false,"success" => true,"msg" => "Physical Examination  Already Exists");
		}
		return $result;
	}	
	...
	public function isexist($master,$id){
		...
		$this->db->where('examination',$master['examination']);
		if($id>0){
			$this->db->where('id !=',$id);	
		}
