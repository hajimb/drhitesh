...
class Lifestyle_model extends CI_Model {
		...
		if($isexists==0){
			if($id ==0){
				...
				if($this->db->trans_status() === FALSE){
				    ...
				    $result = array("status" => true,"success" => true,"msg" => $this->msg." Successfully Added");
				}	
			}else{
				$this->db->where('id', $id);
				...
				if($this->db->trans_status() === FALSE){
				    ...
				    $this->db->trans_commit();
				    $result = array("status" => true,"success" => true,"msg" => $this->msg." Successfully Updated");
				}	
			}
		}else{
			$result = array("status" => false,"success" => true,"msg" => $this->msg." Already Exists");
		}
		return $result;
	}	
	...
	public function isexist($master,$id){
		...
		$this->db->where('lifestyle',$master['lifestyle']);
		if($id>0){
			$this->db->where('id !=',$id);	
		}
