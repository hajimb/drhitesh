...
class Subcategory_model extends CI_Model {
	...
	public function getlist(){
		$sql = "SELECT mb.*,mc.category FROM master_subcategory mb,master_category mc WHERE mb.cid = mc.id";	
	...
	public function isexist($master,$id){
		...
		$this->db->where('cid',$master['cid']);
		$this->db->where('subcategory',$master['subcategory']);
		if($id>0){
			$this->db->where('id !=',$id);	
		}
