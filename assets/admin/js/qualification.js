...
$(document).on("click","#addqualification",function(){
  ...
  $.ajax({
    type    : 'post',
    url     : base_url+'qualification/newqualification',
    data    : formdata,
    cache   : false,
...
function calllist(){
  $('#example1').dataTable( {
    ...
    "sAjaxSource": base_url+'qualification/getqualificationlist/'
  }); 
}
...
$(document).on("click",".GetDetail",function(){
  ...
  $.ajax({
    type    : 'post',
    url     : base_url+'qualification/getqualificationById',
    data    : {id:ID},
    cache   : false,
    success : function(resData){
      console.log(resData);
      ...
      $('#qualification').val(response.data['qualification']);$('#status').val(response.data['status']);$('#deleted').val(response.data['deleted']);$('#addqualification').html('Update');$('#reset').html('Back');
     },
    error : function(error){}
  });
  return false;
...
$(document).on("click","#resetfield",function(){
   $('#qualification').val('');
   $('#status').val(1);
   $('#addqualification').html('Add');$('#reset').html('Reset');
