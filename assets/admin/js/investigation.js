...
$(document).on("click", "#add", function() {
    ...
    $.ajax({
        type: 'post',
        url: base_url + 'investigation/create',
        data: formdata,
        cache: false,
...
function calllist() {
    $('#example1').dataTable({
        ...
        "bSortClasses": false,
        "destroy": true,
        "sAjaxSource": base_url + 'investigation/getlist'
    });
}
...
$(document).on("click", ".GetDetail", function() {
    ...
    $.ajax({
        type: 'post',
        url: base_url + 'investigation/getById',
        data: { id: ID },
        cache: false,
        success: function(resData) {
            console.log(resData);
            ...
            $('#investigation').val(response.data['investigation']);
