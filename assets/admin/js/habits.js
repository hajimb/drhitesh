...
$(document).on("click","#addhabits",function(){
  var formdata = $("#habitsForm").serialize();
  var Id = $("#id").val();
 
  $.ajax({
    type    : 'post',
    url     : base_url+'habits/newhabits',
    data    : formdata,
    cache   : false,
...
function calllist(){
  $('#example1').dataTable( {
    ...
    "sAjaxSource": base_url+'habits/gethabitslist/'
  }); 
}
...
$(document).on("click",".GetDetail",function(){
  ...
  $.ajax({
    type    : 'post',
    url     : base_url+'habits/gethabitsById',
    data    : {id:ID},
    cache   : false,
    success : function(resData){
      console.log(resData);
      ...
      $('#habits').val(response.data['habits']);$('#status').val(response.data['status']);$('#deleted').val(response.data['deleted']);$('#addhabits').html('Update');$('#reset').html('Back');
     },
    error : function(error){}
  });
  return false;
...
$(document).on("click","#resetfield",function(){
   $('#habits').val('');
   $('#status').val(1);
   $('#addhabits').html('Add');$('#reset').html('Reset');
